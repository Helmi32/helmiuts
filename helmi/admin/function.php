...
function query($majalah){
  global $conn;

  $result = mysqli_query($conn, $majalah);
  $majalahb = [];

  while($majalah = mysqli_fetch_assoc($result)){
    $majalahb[]=$majalah;
  }
  return $majalahb;
};
...
function hapus($id){
  ...
  mysqli_query($conn, "DELETE FROM majalah WHERE id = $id");
